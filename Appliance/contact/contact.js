...
var leftInputsBehavior = function(){
    ...
    if(document.body.clientWidth > 1350){
        ...
        $('.leftItem').css('position', 'absolute');

    }
    else{
        $('.leftInputCont').css('width','66.66666667%');
        $('.leftInputCont').css('margin-left','0');
        $('.leftItem').css('position', 'relative');
        if(document.body.clientWidth < 767){
            $('.leftInputCont').css('width','91.66666667%');
        }
    }
};
